        ...
        if len(row_parsed) > 1:
            ...
            if samples is None:
                if exclude is not None:
                    ...
                    neighbors = row_parsed[1:]
                    result_indices.append(indices)  # 没说采样个数 没有排除 就把所有邻居加进去
            else:
                # undersampling frequent neighbors
                unique, counts = np.unique(row_parsed[1:], return_counts=True)
                p = []
        ...
        for adjlist, indices, use_mask in zip(adjlists, edge_metapath_indices_list, use_masks[mode]):
            if use_mask:
                edges, result_indices, num_nodes, mapping = parse_adjlist_LastFM(
                    [adjlist[row[mode]] for row in user_artist_batch], [indices[row[mode]] for row in user_artist_batch]
                    # adjlist[row[mode]是batch中所有mode类点的该元路径下的邻居编号字符串(80 * all_neighbor_num)]
                    # indices[row[mode]]是batch中所有mode类点在该元路径下所有路径节点串(例如v0-v1-0)
                    , samples, user_artist_batch, offset, mode)
            else:
                edges, result_indices, num_nodes, mapping = parse_adjlist_LastFM(
            ...
            # edges是batch中mode类节点采样出来的邻居所构造的图，result_indices是其选的邻居与该点所构成的元路径节点集
...
def cal_loss(pos_embedding_user, pos_embedding_artist,
    ...
    # pos = torch.mean(F.logsigmoid(pos_out))
    # neg = torch.mean(F.logsigmoid(neg_out))
    # lo = torch.mean(pos + neg)
    # return -lo
    return -torch.mean(F.logsigmoid(pos_out) + F.logsigmoid(neg_out))
    # return -torch.mean(torch.mean(F.logsigmoid(pos_out)) + torch.mean(F.logsigmoid(neg_out)))
...
def evaluate(y_proba_test, y_true_test, true_num):
    sorted_pred = y_proba_test.copy()
    sorted_pred.sort()
    threshold = sorted_pred[-true_num]
    y_pred = np.zeros(y_proba_test.size, dtype=np.int32)
    for i in range(y_proba_test.size):
        if y_proba_test[i] >= threshold:
            y_pred[i] = 1

    auc = roc_auc_score(y_true_test, y_pred)
